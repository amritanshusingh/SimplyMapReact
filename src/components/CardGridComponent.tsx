...
const CardGridComponent: React.FC = () => {
  ...
  const cardData = [
    ...
    {
      icon: <AspectRatioIcon sx={{ fontSize: 40, mb: 1 }} />,
      title: "Line to Polygon (KML)",
      description:
        "Get a Polygon data KML file by converting your Line data KML file",
    },
    {
      icon: <BorderStyleIcon sx={{ fontSize: 40, mb: 1 }} />,
      title: "Points to Polygon (KML)",
      description:
        "Get a Polygon data KML file by converting your Points data KML file",
    },
    {
    ...
    >
      {cardData.map((card, index) => (
        <Box
          ...
          onClick={() =>
            navigate(`/${card.title.replace(/\s+/g, "-").toLowerCase()}`)
          }
        >
          <Card
            sx={{
